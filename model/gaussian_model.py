...
class GaussianModel:
    def setup_functions(self, opacity_activation="softplus"):
        ...
        if opacity_activation == "softplus":
            self.density_activation = torch.nn.Softplus()  # use softplus for [0, +inf]
            self.density_inverse_activation = inverse_softplus
        elif opacity_activation == "bounded_sigmoid":
            opacity_min_bound = -0.2
            opacity_max_bound = 1.2
            self.density_activation = (
                    lambda x: torch.sigmoid(x) * (opacity_max_bound - opacity_min_bound)
                    + opacity_min_bound
                )
            self.density_inverse_activation = lambda x: inverse_sigmoid(
                torch.relu((x - opacity_min_bound) / (opacity_max_bound - opacity_min_bound))
            )
        elif opacity_activation == "sigmoid":
            self.density_activation = torch.sigmoid
            self.density_inverse_activation = inverse_sigmoid
        else:
            raise ValueError(f"Invalid opacity activation function: {opacity_activation}")
        print(f"Using opacity activation function: {opacity_activation}")

        self.rotation_activation = torch.nn.functional.normalize

    def __init__(self, scale_bound=None, opacity_activation="softplus"):
        ...
        self.setup_functions(opacity_activation)
    ...
    @property
    def raw_densitys(self) -> torch.Tensor:
        """Returns the Gaussians' unactivated opacities (N, 1)."""
        raw_densitys = self._density
        return raw_densitys
    ...
    def reset_densification_info(self):
        self._densification_info = torch.zeros((2, self._xyz.shape[0]), dtype=torch.float32, device='cuda')
    ...
    def create_from_pcd(self, xyz, density, spatial_lr_scale: float = 1.0):
    ...
    def densify_and_split(self, pts_mask, densify_scale_threshold, N=2):
        n_init_points = self.get_xyz.shape[0]
        # Extract points that satisfy the gradient condition
        selected_pts_mask = torch.zeros((n_init_points), device="cuda").bool()
        selected_pts_mask[: pts_mask.shape[0]] = pts_mask
    ...
    def densify_and_clone(self, pts_mask, densify_scale_threshold):
        # Extract points that satisfy the gradient condition
        selected_pts_mask = torch.logical_and(
            pts_mask,
            torch.max(self.get_scaling, dim=1).values <= densify_scale_threshold,
    ...
    def densify_and_prune(
        ...
        if densify_scale_threshold:
            if not max_num_gaussians or (
                max_num_gaussians and selected_pts_mask.shape[0] < max_num_gaussians
            ):
                self.densify_and_clone(selected_pts_mask, densify_scale_threshold)
                self.densify_and_split(selected_pts_mask, densify_scale_threshold)
    ...
    def add_densification_stats(self, means_grad, update_filter):
        grad_norm = torch.norm(means_grad[update_filter], dim=-1, keepdim=True)
        self.xyz_gradient_accum[update_filter] += grad_norm
        self.denom[update_filter] += 1
    
    def scale_regularization_loss(self, target_ratio=5.0, mode="soft"):
        """
        Penalize overly flat / needle-like Gaussians. Differentiable regularization loss.

        Args:
            target_ratio: Maximum allowed scale ratio.
            mode: "soft" - penalize only when exceeding the threshold;
                  "all" - always penalize anisotropy.

        Returns:
            loss: Regularization loss value.
        """
        scales = self.get_scaling  # (N, 3)

        # Compute anisotropy ratio
        scales_max = scales.max(dim=1).values
        scales_min = scales.min(dim=1).values + 1e-8
        ratio = scales_max / scales_min

        if mode == "soft":
            # Only penalize Gaussians exceeding the threshold (ReLU smooth transition)
            excess = torch.relu(ratio - target_ratio)
            loss = torch.mean(excess ** 2)
        else:
            # Penalize all anisotropy
            loss = torch.mean((ratio - 1.0) ** 2)

        return loss

    def scale_min_regularization(self, min_scale=0.001):
        """
        Penalize overly small scales to prevent Gaussian collapse. Differentiable.
        ...
        scales_min = scales.min(dim=1).values

        # Soft-threshold penalty
        violation = torch.relu(min_scale - scales_min)
